...
void main() {
  final spec = jsonDecode(File('tool/render-openapi.json').readAsStringSync())
  ...
  for (final item in (spec['paths'] as Map<String, dynamic>).values) {
    for (final entry in (item as Map<String, dynamic>).entries) {
      ...
      operationIds
          .add((entry.value as Map<String, dynamic>)['operationId'] as String);
    }
  }

  final expected = operationIds.map(methodName).toSet();

  Set<String> declaredIn(String path, RegExp pattern) {
    final names = <String>{};
  ...
  test('the spec has an operationId for every operation', () {
    ...
    expect(operationIds.toSet(), hasLength(208),
  ...
  test('the flat facade exposes every operation, named as Render names it', () {
    final actual = declaredIn(
      'lib/src/generated/flat.dart',
      RegExp(r'^\s+Future<.*?> ([a-zA-Z_][A-Za-z0-9_]*)\('),
    );
  ...
  test('the vendored spec is the one the code was generated from', () {
    ...
    expect((spec['servers'] as List).first['url'],
